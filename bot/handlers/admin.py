...
# ──────────────────────────────────────────────────────────────
# /test_backup_email <user_id> — test hantar confirmation email
# ──────────────────────────────────────────────────────────────

@router.message(Command("test_backup_email"))
@admin_only
async def cmd_test_backup_email(message: Message):
    from services.email_service import send_confirmation_email, smtp_status

    parts = message.text.split(maxsplit=1)
    if len(parts) < 2 or not parts[1].strip().lstrip("-").isdigit():
        await message.answer(
            "⚠️ Format salah.\n\n"
            "Guna: `/test_backup_email <telegram_user_id>`\n"
            "Contoh: `/test_backup_email 123456789`",
            parse_mode="Markdown",
        )
        return
    ...
    if not email:
        ...
        await message.answer(
            f"⚠️ User *{uname}* (`{target_id}`) belum set backup email.\n\n"
            "Suruh user set dulu melalui ⚙️ Control Panel → 📩 Backup Email.",
            parse_mode="Markdown",
        )
        return
    ...
    wait_msg = await message.answer(
        f"⏳ Menghantar test confirmation email ke `{email}`...", parse_mode="Markdown"
    )

    ok = await send_confirmation_email(
        to_email=email,
        user_id=target_id,
        userbot_id=userbot_id,
        username=username,
    ...
    if ok:
        await message.answer(
            f"✅ *Test confirmation email berjaya dihantar!*\n\n"
            f"📩 To: `{email}`\n"
            f"👤 Telegram: `{target_id}`\n"
            f"🤖 Userbot ID: `{userbot_id or 'Not Set'}`\n\n"
            f"_Semak inbox / spam folder._",
            parse_mode="Markdown",
        )
        await db.write_admin_log(
            ADMIN_ID, "test_backup_email",
            target_user_id=target_id, notes=f"email={email}",
        )
    else:
        await message.answer(
